...
import { vnpayAddress, vnpayHost, vnpaySecureSecret, vnpayTmnCode } from "../secret";
...
export const invoicePaymentStatus = async (req: Request, res: Response) => {
  try {
   ...
   if(verify.isSuccess){
        ...
        await  invoiceRepositoty.update({id : invoiceId},{status : invoiceStatus.PAID})
        res.status(200).json('payment sucessfull !!!')
        return
   }
